...
const APP_VERSION = 'V91 Accurate Source Merge Edition';
...
function dashboard(){
  ...
  $('content').innerHTML = hero('OceanJet PortDesk','Organized V90 build with separated data, cleaner structure, and focused professional port operations.','V90 Merged Complete Clean Professional') + `
    <div class="grid">
...
function downloadScheduleCSV(){ download('OceanJet_V90_Schedule_'+Date.now()+'.csv', buildScheduleCSV(), 'text/csv'); }
...
function exportJSON(){ download('OceanJet_V90_Backup_'+Date.now()+'.json', JSON.stringify({version:APP_VERSION, exportedAt:new Date().toISOString(), state}, null, 2), 'application/json'); }
...
function sync(){
  setTitle('Google Sync Bridge','Manual backup/sync only');
  $('content').innerHTML = hero('Google Sync Bridge','Offline-first app. Google Sync is optional and only runs when staff taps a button.','Manual Sync') + `<div class="grid"><div class="card span6"><label>Apps Script Web App URL<input id="syncUrl" value="${esc(state.data.settings.syncUrl||'')}" placeholder="https://script.google.com/..."></label><div class="row" style="margin-top:12px"><button class="primary" onclick="saveSyncUrl()">Save URL</button><button onclick="testSync()">Test Connection</button><button onclick="downloadSyncPayload()">Download Payload</button></div><div class="notice" style="margin-top:12px">No Google login is forced. Local browser storage remains the main storage.</div></div><div class="card span6"><div class="cardHead"><div><h3>Local Outbox</h3><p>Items queued for manual sync or backup.</p></div><span class="badge ${state.data.syncOutbox.length?'warn':'ok'}">${state.data.syncOutbox.length}</span></div>${outboxHtml()}<div class="row" style="margin-top:12px"><button onclick="downloadSyncPayload()">Download Payload</button><button class="danger" onclick="clearOutbox()">Clear Outbox</button></div></div></div>`;
}
...
function syncPayload(){ return {version:APP_VERSION, generatedAt:new Date().toISOString(), schedules:ROUTES, transactions:state.data.transactions, claims:state.data.claims, evidence:state.data.cases.map(x=>({...x, photos:{stub:!!x.photos?.stub, damage:!!x.photos?.damage}})), audit:state.data.audit, outbox:state.data.syncOutbox}; }
function downloadSyncPayload(){ download('OceanJet_V90_Sync_Payload_'+Date.now()+'.json', JSON.stringify(syncPayload(), null, 2), 'application/json'); }
